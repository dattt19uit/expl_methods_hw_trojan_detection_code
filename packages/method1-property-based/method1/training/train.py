#!/usr/bin/env python3
"""
Property-Based Model Training Script

Trains 31 SVM models (one for each feature combination) using:
- Tuned hyperparameters (C, gamma) from prior hyperparameter tuning
- Class weighting to handle 0.41% trojan imbalance
- Limited parallelism to avoid machine overload
- Comprehensive logging and progress tracking

Based on: xai_train_svm_dynamic.py (core logic)
Enhancements:
  - Dynamic hyperparameter loading from tuning results
  - Automatic class weight computation
  - Limited parallelism (processes=max_cores-2)
...
def setup_logging(log_file):
    ...
    logger.info("="*80)
    logger.info("Property-Based Model Training Started")
...
def rows_to_np(rows, max_vals):
    """Convert rows to normalized numpy array"""
...
def compute_class_weights(labels):
    ...
    Returns:
        dict: Maps class label -> weight (higher weight for minority class)
    """
    ...
    for label, weight in class_weights.items():
        ...
        logger.info(f"  Class {label}: {count:,} samples ({pct:.3f}%), weight={weight:.4f}")
...
def train_single_model(input_data):
    """
    Train a single SVM model for a feature combination
    
    Args:
        input_data: tuple of (model_index, train_data, train_labels, hyperparams)
    
    Returns:
        tuple: (model_index, trained_model, training_metrics)
    """
    model_index, train_data, train_labels, hyperparams = input_data
    
    logger.debug(f"Training model {model_index}: C={hyperparams['C']}, gamma={hyperparams['gamma']}")
    
    # Create and train SVM with class_weight='balanced'
    rbf_svc = SVC(
        kernel='rbf',
        C=hyperparams['C'],
    ...
    pred = rbf_svc.predict(train_data)
    metrics = analyze_results(train_labels, pred, model_index)
    
    return (model_index, rbf_svc, metrics)


def train_models(data_folder, tuning_results_file, output_folder, jobs=None, no_parallel=False):
    """
    Main function to train all 31 property-based SVM models
    
    Args:
        data_folder: Folder with train.csv, test.csv, min.csv, max.csv
        tuning_results_file: Path to hyperparameter tuning results JSON
    ...
    """
    # Setup logging
    log_file = os.path.join(output_folder, 'logs', 'train_models.log')
    ...
    # ========================================================================
    logger.info("Loading hyperparameter tuning results...")
    
    if not os.path.exists(tuning_results_file):
        logger.error(f"Tuning results file not found: {tuning_results_file}")
        raise FileNotFoundError(f"Tuning results: {tuning_results_file}")
    
    with open(tuning_results_file) as f:
        tuning_results = json.load(f)
    
    # Convert tuning results to list of 31 parameter dicts
    # Handle both old format (dict with string keys) and new format (all_properties list)
    c_and_gamma = []
    
    if 'all_properties' in tuning_results:
        # New aggregate tuning format
        logger.info("Detected new aggregate tuning format")
        props_by_idx = {p['property_idx']: p for p in tuning_results['all_properties']}
        for i in range(31):
            if i in props_by_idx:
                c_and_gamma.append({
                    'C': props_by_idx[i]['best_C'],
                    'gamma': props_by_idx[i]['best_gamma']
                })
            else:
                logger.warning(f"Model {i} not found in tuning results, using default")
                c_and_gamma.append({'C': 1.532791659716421, 'gamma': 0.8105480945814617})
    else:
        # Old tuning format (string keys in dict)
        logger.info("Detected old tuning format")
        for i in range(31):
            key = str(i)
            if key in tuning_results:
                c_and_gamma.append({
                    'C': tuning_results[key]['C'],
                    'gamma': tuning_results[key]['gamma']
                })
            else:
                logger.warning(f"Model {i} not found in tuning results, using default")
                c_and_gamma.append({'C': 1.532791659716421, 'gamma': 0.8105480945814617})
    
    logger.info(f"Loaded hyperparameters for 31 models")
    for i, params in enumerate(c_and_gamma):
        logger.debug(f"  Model {i}: C={params['C']:.6f}, gamma={params['gamma']:.6f}")
    ...
    # ========================================================================
    logger.info("Converting to numpy arrays and scaling...")
    ...
    logger.info(f"  Training data shape: {train_data_scaled.shape}")
    logger.info(f"  Test data shape: {test_data_scaled.shape}")
    ...
    n_features = len(train_data_scaled[0])
    ...
    for property in properties:
        data = np.array([row[list(property)] for row in train_data_scaled])
        properties_training_data.append(data)
    
    properties_test_data = []
    for property in properties:
        data = np.array([row[list(property)] for row in test_data_scaled])
    ...
    # ========================================================================
    logger.info("="*80)
    logger.info(f"Training 31 models with {n_jobs} parallel processes...")
    logger.info("="*80)
    ...
    train_inputs = [
        (i, properties_training_data[i], training_no_dupes_labels, c_and_gamma[i])
        for i in range(31)
    ]
    ...
    output_files = {
        ...
        'hyperparameters.json': c_and_gamma,
        ...
        'scaled_training_data.json': train_data_scaled.tolist(),
        'scaled_test_data.json': test_data_scaled.tolist(),
        'test_data.json': test,
    }
