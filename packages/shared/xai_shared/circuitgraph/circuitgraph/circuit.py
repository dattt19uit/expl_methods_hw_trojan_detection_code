...
supported_types = addable_types + ["bb_input", "bb_output", 'wire']
...
class Circuit:
    ...
    def __init__(self, name=None, graph=None, blackboxes=None):
        ...
        if graph:                        # c.graph.nodes, c.graph.nodes['node']=>attributes, c.graph.nodes['node']['attr']=value
            self.graph = graph
        else:
            self.graph = nx.DiGraph()    # c.graph.edges, c.graph.edges(node)

        # c.blackboxes, c.blackboxes[Ux].name==>NAND2X1, c.blackboxes[Ux].inputs(), c.blackboxes[Ux].outputs()
        if blackboxes:
    ...
    def add_blackbox(self, blackbox, name, connections=None):
        ...
        # make nodes
        io = []
        #for n in blackbox.inputs():                      # add a node even if there is no connection
        #    io += [self.add(f"{name}.{n}", "bb_input")]  # self.add(n="U7.A", node_type="bb_input")
        #for n in blackbox.outputs():
        #    io += [self.add(f"{name}.{n}", "bb_output")] # self.add(n="U7.Y", node_type="bb_output")
        # print("io=", io) # io= ['U7.A', 'U7.B', 'U7.Y']

        # make connections
        if connections:
            for bb_n, ns in connections.items():
                if bb_n in blackbox.inputs():
                    io += [self.add(f"{name}.{bb_n}", "bb_input")]  # new, add only what is needed, primitive gates
                    self.connect(ns, f"{name}.{bb_n}")
                elif bb_n in blackbox.outputs():
                    io += [self.add(f"{name}.{bb_n}", "bb_output")] # new, add only what is needed, primitive gates
    ...
    def add(
        ...
        if node_type not in supported_types: ## NEW
    ...
    def connect(self, us, vs):
        ...
        for u in us:
            ...
            if t in ["bb_output"]:
                for v in vs:
                    if self.type(v) != "buf" and self.type(v) != "wire":    ## old "buf"
                        raise ValueError(
                            f"cannot connect from {t} '{u}' to non-buf or wire '{v}'" ## 'or wire'
                        )
    ...
    def set_output(self, ns, output=True):
        ...
        """
        if isinstance(ns, str):
            ns = [ns]               # if string then convert to a list

        #print('*** set_output:output,ns=', output, ns)
        #print('*** set_output:self.graph.nodes=', self.graph.nodes)
        for n in ns:                # ['rec_dataH[0]']
            #print('   set_output n=', n)
            self.graph.nodes[n]["output"] = output
            #self.graph.nodes[n]["output"] = True
            #print('nodes[n]["output"]=', self.graph.nodes[n]["output"])
...
class BlackBox:
    ...
    def __init__(self, name=None, inputs=None, outputs=None):
        ...
        """
        self.name = name
        # Preserve input and output order as lists for positional port mapping
        self.input_list = list(inputs) if inputs else []
        self.output_list = list(outputs) if outputs else []
        # Keep sets for backward compatibility
        self.input_set = set(self.input_list)
        self.output_set = set(self.output_list)
    ...
    def inputs_ordered(self):
        """
        Return the blackbox's inputs in definition order.

        Returns
        -------
        list of str
                Input nodes in blackbox (preserves order).

        """
        return self.input_list
    
    def outputs_ordered(self):
        """
        Return the blackbox's outputs in definition order.

        Returns
        -------
        list of str
                Output nodes in blackbox (preserves order).
